...
void Instance::readFromStream(std::istream& is)
{
	...
	is.read((char*)&z, sizeof(unsigned int));
	is.read((char*)&m, sizeof(unsigned int));
	...
	for(i=0; i<n; ++i)
	{
		is.read((char*)&nextl, sizeof(unsigned int));
		if(nextl > 0)
		{
			boost::dynamic_bitset<> bs;
			bs.resize(nextl);
...
void Instance::writeToStream(std::ostream& os) const
{
	os << "PAR";
...
void Instance::serialize(ostream& os) const
{
	writeToStream(os);
}
